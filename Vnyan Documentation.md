...
Munch Speaks! | 🟩 | 🟩 | Zawarudokillswitch | Triggers the Squawk TTS and reads out whatever the user has entered into the redeem text field. In order for this action to function as intended, it must be used in tandem with my streamer bot logic.
...
Fuzzy Fact | 🟩 | 🟥 | Zawarudokillswitch | Used to take user input when redeemed and share it via Squawk TTS in chat when set up as an OBS audio source. In order for this action to function as intended, it must be used in tandem with my streamer bot logic.The !fuzzyfact command in streamerbot will need to have its execute code modified with any additional lines as they are not automatically appended to the code logic.
...
Question | 🟩 | 🟥 | Zawarudokillswitch | This command issues a randomly chosen question to the streamer and also triggers TTS if Squawk TTS is configured. It will also unmute the TTS source if muted and set certain OBS Filters to match. In order for this action to function as intended, it must be used in tandem with my streamer bot logic.
Fanart! | 🟩 | 🟩 | Zawarudokillswitch | Used to trigger confetti in vnyan when the Fanart! redeem is used. Requires my StreamerBot logic to function as intended.
...
Walkons | 🟩 | 🟩 | ItzapixFanfare, GranethFanfare, TopHatTigFanfare, domtherizzlybearfanfare | Triggers a walkon fanfar once when the specified user creates a twitch chat message. There's lots of ways to handle this, and this serves as an example of how to do so in vnyan. The parameters used are set to 0 when vnyan is launched.
...
On Subscription | 🟩 | 🟥 | SubscriptionRainActive | Triggers a screenwide particle effect when a sub is received. Disables it after the specified amount of time has passed. Requires my Streamerbot logic to function as intended.
Hype Train Events | 🟩 | 🟩 | HypeParticleActive | Triggers a screenwide particle effect when a hype train is active. Disables it when the hypetrain ends. Requires my Streamerbot logic to function as intended.
...
On Follow | 🟩 | 🟥 | LightFormActive, RHDrawActive, LHDrawActive, wereberyloaded | Triggers a dancing animation, sets the specified blendshape values, swaps cameras, triggers a particle effect, and toggles certain props if active based off the current vtuber in use when a follow event occurs. Requires my Streamerbot logic to function as intended.
...
Grizz earthquake nut | 🟩 | 🟩 | nuttoss, GrizzEarthquakeActive, EarthquakeMultiplier | Another fun one! This one works by applying small increments of force to the currently active model to give it the illusion of tumbling in an earthquake, while also dropping the specified props that gradually scale in size according to the intensity of the camera tilt and value of the associated parameter.
...
Kodai Ryu Flaming Book Nut | 🟩 | 🟩 | | This will drop the specified droppable after the delay and play an SFX when the object hits while also randomizing another SFX from 5 possible outcomes.
